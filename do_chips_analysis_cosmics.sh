...
default_source="cosmics"
default_kind_of_detector="module_chips_detector"
default_nevents=480000000
default_nevents_per_job=4800000
default_nevents_per_hour=2400000
...
read_kind_of_detector(){
 ...
 if [ "${default_kind_of_detector}" == "full_chips_detector" ]; then
     g4runfile_base="run_cosmic__full_chips_detector.mac"
     g4cmdfile_base="cmd__full_chips_detector.file"
 elif [ "${default_kind_of_detector}" == "module_chips_detector" ]; then
     g4runfile_base="run_cosmic__module_chips_detector.mac"
     g4cmdfile_base="cmd__module_chips_detector.file"
 else
...
return 1
 ...
 for (( c=1; c<=$njobs; c++ ))
   ...
   cd ${CHIPS_DIR} 
   cp *mac ${OUTPUT_DIR}_${FLAG}/
   cp ${g4runfile} ${OUTPUT_DIR}_${FLAG}/
   cp ${g4cmdfile_base} ${OUTPUT_DIR}_${FLAG}/
